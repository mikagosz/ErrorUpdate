...
Removes every report, the raw crash file, cached downloads and all
`ErrorUpdate_*` defaults. Use it behind a "delete my diagnostic data" button, and
**call it before removing the framework from your app** — otherwise the reports
outlive the integration on your users' disks. See *Removing the Framework* in
...
- **An update that installs but changes nothing is reported, not repeated.**
  ...
  installed; newer versions are never affected.
- **A cached periodic check does not clear `availableUpdate`.** A check the
  1-hour cache skips makes no request, so it reports nothing about the server;
  only a check that reached it can decide there is no update.
