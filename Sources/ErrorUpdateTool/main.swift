...
func printUsage() {
    ...
    EXAMPLE:
      ...
      errorupdate-tool release \\
          ...
          --notes "Bug fixes" --out www/api/error-update/version-check
    """)
}
...
func runKeygen() {
    ...
    Generated an Ed25519 key pair:
      private key: \(privateKeyURL.path)   <-- KEEP THIS SECRET, never commit it
      public key:  \(publicKeyURL.path)

    Configure the public key in your app like this:
...
func runRelease() {
    ...
    if signature.isEmpty {
        FileHandle.standardError.write(Data("""
            WARNING: manifest without a signature. An app with a public key configured \
            will refuse this update, and an app without one will accept whatever file \
            that server offers. Sign the release with --key.\n
            """.utf8))
    }

    print("""
    Manifest written: \(outURL.path)
      version:   \(version)
      sha256:    \(sha256)
      signature: \(signature.isEmpty ? "(none — pass --key to sign)" : "Ed25519 OK")
      available: \(hasFlag("--unavailable") ? "no (--unavailable)" : "yes")
    """)
}
