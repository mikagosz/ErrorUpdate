...
public final class ReportStore: @unchecked Sendable {
    ...
    /// Deletes every stored report and the directory holding them.
    ///
    /// Part of the framework's way out: a library that writes to the user's disk
    /// has to offer a way to take it all back. Reports carry stack traces with
    /// `/Users/<name>/` paths and hardware details, and without this they outlive
    /// the integration by years.
    ///
    /// The directory is recreated empty, so the store stays usable afterwards.
    ///
    /// Runs on the store's queue like every other operation, and that is the
    /// point: `save(_:)` writes asynchronously, so erasing off-queue could
    /// overtake a write still in flight and the report would reappear right after
    /// being deleted. (Measured — it is what made this method's own test flaky
    /// under a full test run.) Going through the queue also keeps the promise
    /// that `cachedReports` and `idsByContentHash` are touched nowhere else.
