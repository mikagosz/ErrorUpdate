...
@Suite struct EndToEndTests {
    ...
    func fullUpdateCycle() async throws {
        ...
        let info = try #require(try await checker.checkForUpdates(force: true).info)
        ...
        // Without a reference the installer skips the identifier and signature
        // checks, because the test host is not an .app bundle. Pointing at the
        // built demo app makes the full cycle go through `codesign -R` too.
