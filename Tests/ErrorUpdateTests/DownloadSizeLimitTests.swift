...
@Suite(.serialized) struct DownloadSizeLimitTests {
    ...
    func announcedContentLengthOverLimit_abortsBeforeDownloadingEverything() async throws {
        ...
        // The whole point: the file never reached the user's disk in full.
        #expect(server.bytesSent < payloadSize,
                "The server sent \(server.bytesSent) of \(payloadSize) bytes — the download was not aborted")
    }
    ...
    // MARK: 3. A file within the limit travels the same path
