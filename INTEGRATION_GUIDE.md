...
When the user sends a report by mail, it travels as a **file attachment**
(`ErrorReport-<version>-<date>.txt`), so they can open it and see exactly what
is leaving their machine before hitting send. A crash report runs to tens of
kilobytes, which no `mailto:` link can carry; if the system's mail composer is
unavailable, the framework falls back to `mailto:` with the full text on the
clipboard.
...
A cached check makes no request, and that silence is **not** an answer that
there is no update: `availableUpdate` is left exactly as it was. Only a check
that actually reached the server can clear it. Starting the schedule right after
...
nothing — almost always because the packaged build kept its old
`CFBundleShortVersionString` while the manifest promised a new one. Left alone,
every later check offers that same update again, forever.

The framework notices on the next launch and:

- publishes `ErrorUpdateManager.shared.ineffectiveUpdate` (an `IneffectiveUpdate`
  with `expectedVersion`, `actualVersion` and a ready-made `localizedDescription`),
- calls the delegate's `updateDidNotTakeEffect(_:)`,
...
```swift
if let problem = manager.ineffectiveUpdate {
    Text(problem.localizedDescription)   // aimed at whoever built the release
}
```
...
class MyDelegate: ErrorUpdateDelegate {
    ...
    // An install that changed nothing — a packaging mistake, not a runtime
    // error, which is why it does not come through updateDidFail.
    func updateDidNotTakeEffect(_ report: IneffectiveUpdate) {
        print("Installed \(report.expectedVersion), still running \(report.actualVersion)")
    }
}
...
## 7. Keeping the Integration Removable

Put the whole integration in **one file** — configuration, delegate, and any
diagnostic UI — and leave the app's own scene with just the call that starts it:

```swift
// ErrorUpdateIntegration.swift — everything framework-related lives here
enum ErrorUpdateSetup {
    static func configure() { … }
}

// YourApp.swift — one line
func applicationDidFinishLaunching(_ notification: Notification) {
    ErrorUpdateSetup.configure()
}
```

This is not style advice. Measured on two applications, removing the framework
laid out this way cost **one file, 11–15 lines, and the package reference** —
after which the project built cleanly and the binary contained no trace of it.
Configuration scattered across view code is what makes a library hard to leave.

## 8. Removing the Framework

Taking the code out is only half of it. The framework writes to the user's disk,
and those files outlive the integration unless you erase them.

**Before you drop the dependency**, run:

```swift
ErrorUpdateManager.shared.eraseAllStoredData()
```

It removes stored reports, the raw crash file and its quarantined sibling, cached
downloads, and every `ErrorUpdate_*` key in `UserDefaults`. It returns `false` if
anything survived. The same call gives your users a "delete my diagnostic data"
button — worth having, since reports contain stack traces with `/Users/<name>/`
paths and hardware details.

What is left behind if you skip it:

| Location | What |
|---|---|
| `~/Library/Application Support/<bundle id>/reports/` | stored reports (up to 50) |
| `~/Library/Application Support/<bundle id>/crash.raw` | an unprocessed crash, if any |
| `UserDefaults` | `ErrorUpdate_*` keys (skipped version, last check, install verdict) |
| temporary directory | `ErrorUpdate_download/`, cleared by the system eventually |

Then remove the integration file, the call in your app delegate, and the package
reference in Xcode.
